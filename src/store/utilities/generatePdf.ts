...
export const generatePDF = async (slides: Slide[]): Promise<Blob> => {
...
function addTextToPDF(pdf: jsPDF, element: SlideElement) {
    if (element.type === "SlideText") {
        ...
        const textX = element.pos.ox;
        const textY = element.pos.oy + element.fontSize;
...
function addImageToPDF(pdf: jsPDF, element: SlideElement) {
    if (element.type === "SlideImage") {
        ...
        pdf.addImage(img, "JPEG", element.pos.ox, element.pos.oy, element.size.width, element.size.height);
    }
}